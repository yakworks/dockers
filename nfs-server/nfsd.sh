...
if [ -z "${SHARED_DIRECTORY}" ]; then
  echo "The SHARED_DIRECTORY environment variable is unset or null"
  exit 1
fi

SHARED_PATH="/nfs-volume/${SHARED_DIRECTORY}"
echo "sharing SHARED_PATH: $SHARED_PATH"

if [ ! -e "${SHARED_PATH}" ]; then
  echo "$SHARED_PATH does not exist, creating ..."
  mkdir -p $SHARED_PATH
  chown nobody:users $SHARED_PATH
  chmod -R 777 $SHARED_PATH 
fi

echo "Writing SHARED_PATH to /etc/exports file"
/bin/sed -i "s@{{SHARED_DIRECTORY}}@${SHARED_PATH}@g" /etc/exports
